...
import { redoAction, undoAction } from "../../reducers/enhancers/undoable/undoable-actions";
import { canStateRedo, canStateUndo } from "../../reducers/enhancers/undoable/undoable-selectors";
...
export const HeaderContainer: React.FC = () => {
  ...
  const canRedo = useSelector((store: StoreState) => canStateRedo(store.filesAndFolders));
  const canUndo = useSelector((store: StoreState) => canStateUndo(store.filesAndFolders));
  ...
  const resetWorkspace = useCallback(() => dispatch(resetStoreThunk()), [dispatch]);
