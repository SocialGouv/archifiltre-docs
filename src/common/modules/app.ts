import { app, type Dialog, dialog, type Shell, shell } from "electron";
...
declare module "../ipc/event" {
  ...
  interface AsyncIpcMapping {
    "dialog.showMessageBox": IpcConfig<Parameters<Dialog["showMessageBox"]>, ReturnType<Dialog["showMessageBox"]>>;
    ...
    "shell.openExternal": IpcConfig<Parameters<Shell["openExternal"]>, ReturnType<Shell["openExternal"]>>;
...
export const loadApp = (): void => {
  ...
  ipcMain.handle("dialog.showMessageBox", async (_event, options) => dialog.showMessageBox(options));
  ipcMain.handle("dialog.showOpenDialog", async (_event, options) => dialog.showOpenDialog(options));
  ipcMain.handle("dialog.showSaveDialog", async (_event, options) => dialog.showSaveDialog(options));
  ipcMain.handle("shell.openPath", async (_event, path) => shell.openPath(path));
  ipcMain.handle("shell.showItemInFolder", (_event, fullPath) => {
    shell.showItemInFolder(fullPath);
  });
  ipcMain.handle("shell.openExternal", async (_event, fullPath, options) => shell.openExternal(fullPath, options));
};
