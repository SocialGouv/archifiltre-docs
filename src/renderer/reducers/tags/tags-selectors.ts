...
import { type Tag, type TagMap } from "./tags-types";
 ...
 */
export const sortTags = (tagList: Tag[], options = defaultSortTagOptions): Tag[] => {
...
export const getTagSize = (
  ...
  const parentIsTagged = (taggedFfidsList: string[], potentialChildId: string) =>
    taggedFfidsList.some(
      potentialParentId =>
        potentialChildId.startsWith(potentialParentId) && potentialChildId[potentialParentId.length] === "/",
    );

  const taggedFfIds = tag.ffIds.filter(id => !parentIsTagged(tag.ffIds, id));

  return taggedFfIds.reduce((totalSize, id) => totalSize + filesAndFoldersMetadata[id].childrenTotalSize, 0);
};

export const tagMapToArray = (tagMap: TagMap): Tag[] => Object.values(tagMap);

export const tagHasFfId = (tag: Tag, ffId: string): boolean => tag.ffIds.includes(ffId);

/**
 * Gets the tags map from the redux store.
 * @param store The redux store
 */
export const getTagsFromStore = (store: StoreState): TagMap => getCurrentState(store.tags).tags;
