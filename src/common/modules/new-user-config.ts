...
function ensureInited(_store = store): asserts _store is Store<UserConfigObject> {
  if (!inited) {
...
declare module "../ipc/event" {
  interface SyncIpcMapping {
    ...
    "user-config.set": {
      [K in keyof UserConfigObject]: IpcConfig<[key: K, value: UserConfigObject[K]], UserConfigObject[K]>;
    }[keyof UserConfigObject];
  }
}
...
export function get<TKey extends keyof UserConfigObject>(key: TKey): UserConfigObject[TKey] {
  if (IS_MAIN) {
    ensureInited(store);
...
export function set<TKey extends keyof UserConfigObject>(
  ...
  return ipcRenderer.sendSync("user-config.set", key as never, value as never) as UserConfigObject[TKey];
}
