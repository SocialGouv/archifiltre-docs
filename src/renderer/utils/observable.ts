import { identity, merge, type Observable, type OperatorFunction, partition } from "rxjs";
...
export const operateOnDataProcessingStream = <TInput, TOutput>(
  ...
  { error: errorOperator = identity, result: resultOperator }: DataProcessingStreamOperators<TInput, TOutput>,
): DataProcessingStream<TOutput> => {
  const [results$, errors$] = partition(
    status$,
    (message): message is ResultMessage<TInput> => message.type === MessageTypes.RESULT,
  ) as [Observable<DataProcessingResult<TInput>>, Observable<DataProcessingError>];
