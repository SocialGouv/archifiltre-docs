...
import { ArchifiltreFileSystemErrorCode, UnknownError } from "../error/error-codes";
...
export const isResult = (value: HashComputingResult | HashComputingError): value is HashComputingResult =>
    value.type === "result";
...
export const computeHash = async (filePath: string): Promise<HashComputingResult | HashComputingError> => {
    try {
...
export const computeHashes = (files: string[], basePath: string) => {
    const computeFn = computeQueue(computeHash, isResult);

    const paths = files.map(file => path.join(basePath, file));

    return computeFn(paths)
...
export const hashErrorToArchifiltreError = (hashError: HashComputingError): ArchifiltreError => ({
