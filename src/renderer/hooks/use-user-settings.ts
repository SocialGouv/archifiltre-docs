...
interface UserSettingsProps {
  updateUserSettings: (userSettings: Partial<UserLocalSettings>) => Promise<void>;
  userSettings: UserLocalSettings;
}

export const useGetUserSettings = (): UserSettingsProps => {
  const [userSettings, setUserSettings] = useState<UserLocalSettings>(getInitialUserLocalSettings());

  const updateUserSettings = async (_newUserSettings: Partial<UserLocalSettings>) => {
