...
const recCp = (hook,old_path,new_path) => {
  ...
  if (stats.isDirectory()) {
    ...
    Fs.readdirSync(old_path)
      .forEach(a=>recCp(hook,Path.join(old_path,a),Path.join(new_path,a)))
  } else {
    hook()
...
const recAddToPack = (hook,path,name,pack) => {
  return new Promise((resolve,reject) => {
    ...
    if (stats.isDirectory()) {
      Fs.readdirSync(path)
        .map(a=>()=>recAddToPack(hook,Path.join(path,a),Path.join(name,a),pack))
...
export const untar = (hook,old_path,new_path) => {
  return new Promise((resolve,reject) => {
    ...
    inp_stream.pipe(TarFs.extract(new_path,options))
    .on('finish',()=>{
      resolve()
    })
  })
