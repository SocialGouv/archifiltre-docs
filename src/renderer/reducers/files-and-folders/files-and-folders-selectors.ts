...
import { type Mapper, not, size } from "@common/utils/functionnal-programming";
 ...
 */
export const getFilesAndFoldersFromStore = (store: StoreState): FilesAndFoldersMap =>
  getCurrentState(store.filesAndFolders).filesAndFolders;
 ...
 * @param store
 */
export const getLastModifiedDateOverrides = (store: StoreState): LastModifiedMap =>
  getCurrentState(store.filesAndFolders).overrideLastModified;

export const useLastModifiedDateOverrides = (): LastModifiedMap => useSelector(getLastModifiedDateOverrides);
...
export const getFilesTotalCount = (
  ...
  return elementsIds.reduce((count, elementId) => count + filesAndFoldersMetadata[elementId]?.nbChildrenFiles, 0);
};
...
export const getFilesTotalSize = (
  ...
  const filteredElementsIds = excludeChildNodes(elementsIds, filesAndFoldersMap);
  return filteredElementsIds.reduce(
    (count, elementId) => count + filesAndFoldersMetadata[elementId]?.childrenTotalSize,
    0,
  );
};
...
const excludeChildrenNodesRec = (
  ...
  return children.reduce(
    (previousChildId, childId) => excludeChildrenNodesRec(previousChildId, filesAndFoldersMap, childId),
    nextElementIds,
  );
};
 ...
 */
export const excludeChildNodes = (elementIds: string[], filesAndFolders: FilesAndFoldersMap): string[] =>
  elementIds.reduce(
    (nextElementIds, elementId) =>
      filesAndFolders[elementId].children.reduce(
        (acc, childId) => excludeChildrenNodesRec(acc, filesAndFolders, childId),
        nextElementIds,
      ),
    elementIds,
  );
 ...
 * @param store
 */
export const getVirtualPathToIdFromStore = (store: StoreState): VirtualPathToIdMap =>
  getCurrentState(store.filesAndFolders).virtualPathToId;
...
export const reduceFilesAndFolders = <TReduceResult>(
  filesAndFoldersMap: FilesAndFoldersMap,
  rootId: string,
  reducer: (childrenValues: TReduceResult[], currentFilesAndFolders: FilesAndFolders) => TReduceResult,
): TReduceResult => {
 ...
 */
const getAllLastModified = (filesAndFoldersMap: FilesAndFoldersMap, filesAndFoldersId: string): number[] =>
  reduceFilesAndFolders(filesAndFoldersMap, filesAndFoldersId, (childrenValues, currentFilesAndFolders) => {
    if (currentFilesAndFolders.file_last_modified === 0) {
      return _.flatten(childrenValues);
    } else {
      return [..._.flatten(childrenValues), currentFilesAndFolders.file_last_modified];
    }
  });
...
export const getFilesAndFoldersMedianLastModified = (
  ...
  medianOnSortedArray(
    getAllLastModified(filesAndFoldersMap, filesAndFoldersId).sort((value1, value2) => value1 - value2),
  );
 ...
 */
export const getDepthFromPath = (filesAndFoldersId: string): number => filesAndFoldersId.split("/").length - 2;

/**
 * Gets the comments map from the redux state
 */
export const getCommentsFromStore = (store: StoreState): CommentsMap => getCurrentState(store.filesAndFolders).comments;

/**
 * Gets the aliases map from the redux state
 */
export const getAliasesFromStore = (store: StoreState): AliasMap => getCurrentState(store.filesAndFolders).aliases;

/**
 * Get only files from files and folders
 */
export const getFilesMap: Mapper<FilesAndFoldersMap, FilesAndFoldersMap> = fp.pickBy(isFile);

/**
 * Get only folders from files and folders
 */
export const getFoldersMap: Mapper<FilesAndFoldersMap, FilesAndFoldersMap> = fp.pickBy(fp.compose([not, isFile]));
 ...
 */
export function getArchiveFoldersCount(filesAndFoldersMap: FilesAndFoldersMap): number {
...
export const getMaxDepth = (filesAndFoldersMap: FilesAndFoldersMap): number =>
  reduceFilesAndFolders(filesAndFoldersMap, "", (childrenDepth: number[], currentFilesAndFolders) => {
...
export const findElementParent = (childId: string, filesAndFolders: FilesAndFoldersMap): FilesAndFolders | undefined =>
 ...
 */
export const getAreHashesReady = (store: StoreState): boolean => getHashesFromStore(store)[ROOT_FF_ID] !== undefined;

/**
 * Get the registered errored files and folders
 */
export const getErroredFilesAndFolders = (store: StoreState): ArchifiltreDocsError[] =>
  getCurrentState(store.filesAndFolders).erroredFilesAndFolders;

export const useFilesAndFoldersErrors = (): ArchifiltreDocsError[] => useSelector(getErroredFilesAndFolders);
